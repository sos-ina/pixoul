...
export default function ReviewsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#060606] text-white pt-28 pb-20 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-1/3 space-y-6">
            <div className="bg-[#111] border border-white/5 rounded-3xl p-8 sticky top-32">
              ...
              <button 
                ...
                className="w-full py-4 bg-[#38C2D9] text-black font-black rounded-xl hover:scale-[1.02] transition-transform uppercase italic"
              >
                Write a Review
              </button>
              ...
              {!isLoggedIn || isUnauthorized ? (
                <button
                  ...
                  className="w-full mt-3 py-3 border border-white/10 text-white/80 font-bold rounded-xl hover:text-white"
                >
          ...
          <main className="lg:w-2/3 space-y-4">
            {isLoading ? (
              <div className="bg-[#111] border border-white/5 rounded-3xl p-6 text-gray-400">
                Loading reviews...
              </div>
            ) : null}
...
function ReviewCard({ name, date, tag, text, initials, rating = 5, bgColor = "bg-blue-500" }) {
  const stars = "★★★★★".slice(0, Math.max(0, Math.min(5, Math.round(Number(rating) || 0))));
  return (
    <div className="bg-[#111] border border-white/5 rounded-3xl p-6 hover:border-white/10 transition-all">
...
function ReviewModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      {/* Form Container */}
      <div className="relative bg-[#0a0a0a] border border-[#38C2D9]/30 w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl shadow-[#38C2D9]/10">
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2 text-3xl text-gray-700">
              {[1, 2, 3, 4, 5].map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() => setRating(s)}
                  className={`${s <= rating ? "text-yellow-400" : "text-gray-700"} hover:text-yellow-400 transition-colors`}
                >
                  ★
                </button>
              ))}
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-[#111] border border-white/10 rounded-xl p-4 text-sm text-white focus:border-[#38C2D9] outline-none appearance-none cursor-pointer"
            >
              <option className="bg-[#111] text-gray-400" value="">Select an experience...</option>
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 h-40 text-sm focus:border-[#38C2D9] outline-none transition-all resize-none"
            />
          </div>
          ...
          <div className="flex gap-4 pt-2">
            <button
              ...
              onClick={onPublish}
              disabled={isLoading}
              className="flex-1 py-4 bg-[#38C2D9] text-black font-black rounded-xl italic uppercase hover:scale-[1.02] transition-transform disabled:opacity-50"
            >
              Publish Review
            </button>
            <button type="button" onClick={onClose} className="px-6 text-gray-500 font-bold text-xs uppercase hover:text-white transition-colors">
              Cancel
            </button>
          </div>
        </div>
