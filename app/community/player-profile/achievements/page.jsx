...
export default function AchievementGallery() {
  ...
  const getTierColor = (tier) => {
    switch (tier) {
      ...
      default: return "text-black/60 dark:text-gray-300 border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5";
    }
  };
    ...
    <div className="min-h-screen dark:bg-[#0a0a0a]  dark:text-white text-black bg-white pt-28 px-6 pb-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <Link href="/community/player-profile" className="text-[10px] font-black uppercase text-black/60 dark:text-gray-500 hover:text-[#38C2D9] mb-4 block transition-all">
              ← Back to Profile
            </Link>
            <h1 className="text-6xl font-black uppercase tracking-tighter">Trophy <span className="text-[#38C2D9]">Case</span></h1>
          </div>
          <div className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-none px-6 py-4 flex gap-8">
            <div className="text-center">
              <p className="text-2xl font-black italic">52</p>
              <p className="text-[8px] uppercase font-black text-black/60 dark:text-gray-500">Unlocked</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-black italic text-yellow-500">3</p>
              <p className="text-[8px] uppercase font-black text-black/60 dark:text-gray-500">Legendary</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {achievements.map((ach) => (
            <div 
              key={ach.id} 
              className={`p-6 rounded-none border shadow-sm transition-all hover:scale-[1.02] ${ach.date === 'Locked' ? 'opacity-40 grayscale' : 'hover:shadow-[0_0_30px_rgba(56,194,217,0.1)]'} ${getTierColor(ach.tier)}`}
            >
              <div className="flex justify-between items-start mb-6">
                <span className="text-4xl">{ach.icon}</span>
                <span className="text-[8px] font-black uppercase tracking-widest px-2 py-1 border border-current rounded-none">
                  {ach.tier}
                </span>
              </div>
              
              <h3 className="text-xl font-black uppercase italic mb-1 dark:text-white text-black">{ach.title}</h3>
              <p className="text-xs text-black/60 dark:text-gray-400 font-medium leading-relaxed mb-4">{ach.desc}</p>
              
              <div className="pt-4 border-t border-black/10 dark:border-white/10 flex justify-between items-center">
                <span className="text-[9px] font-black uppercase text-black/60 dark:text-gray-500">Earned On</span>
